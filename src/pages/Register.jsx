...
const ROLES = [
  { id: 'student',  icon: '🎓', label: 'Student',  desc: 'Access and share my certificates', color: '#38bdf8' },
  { id: 'employer', icon: '🏢', label: 'Employer',  desc: 'Verify candidate credentials',     color: '#00d4aa' },
]

const ROLE_ROUTES = { student: '/student', employer: '/employer' }
...
export default function Register() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const payload = {
        ...
        role:     selectedRole,
        ...(selectedRole === 'student'  && { course: data.course || undefined, studentId: data.studentId || undefined }),
        ...(selectedRole === 'employer' && { company: data.company || undefined, designation: data.designation || undefined }),
      }

      // ✅ Calls POST /api/auth/register, stores token, sets user in context
  ...
  const handleRoleChange = (id) => {
    setSelectedRole(id)
    reset({ course: '', studentId: '', company: '', designation: '' })
  }
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center px-6 py-20 relative overflow-hidden" style={{marginTop:"-70px"}}>
      <div className="absolute -top-20 -right-20 w-96 h-96 rounded-full opacity-50 pointer-events-none"
        style={{ background: `radial-gradient(circle, ${role.color}18 0%, transparent 70%)` }} />
      ...
      <div className="w-full max-w-[480px] relative z-10">
        ...
        <div className="mb-8">
          ...
          <p className="text-[15px] text-white/40">Join CredChain and start managing certificates</p>
        </div>

        {/* Role selector */}
        <div className="mb-6">
          <label className="block text-[11px] font-medium tracking-[0.06em] uppercase text-white/35 mb-3">I am a</label>
          <div className="grid grid-cols-2 gap-3">
            {ROLES.map(r => (
              <motion.button key={r.id} type="button" onClick={() => handleRoleChange(r.id)}
                data-hover whileTap={{ scale: 0.98 }}
                className="relative text-left p-5 rounded-2xl border transition-all duration-200"
                style={{
                  background:  selectedRole === r.id ? `${r.color}0a` : '#0c0c0c',
                  borderColor: selectedRole === r.id ? `${r.color}45` : 'rgba(255,255,255,0.07)',
                }}
              >
                {selectedRole === r.id && (
                  <motion.div layoutId="roleActiveBorder" className="absolute inset-0 rounded-2xl"
                    style={{ boxShadow: `inset 0 0 0 1px ${r.color}50` }} />
                )}
                <AnimatePresence>
                  {selectedRole === r.id && (
                    <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }}
                      ...
                      style={{ background: r.color, color: 'black' }}>✓</motion.div>
                  )}
                </AnimatePresence>
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl mb-3 border"
                  style={{
                    background:  selectedRole === r.id ? `${r.color}15` : 'rgba(255,255,255,0.04)',
                    borderColor: selectedRole === r.id ? `${r.color}30` : 'rgba(255,255,255,0.08)',
                  }}>{r.icon}</div>
                <div className="text-[14px] font-[600] tracking-[-0.01em] mb-1"
                  style={{ color: selectedRole === r.id ? 'white' : 'rgba(255,255,255,0.6)' }}>{r.label}</div>
                <div className="text-[11px] text-white/30 leading-relaxed">{r.desc}</div>
              </motion.button>
            ))}
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4" noValidate>
            ...
            <AnimatePresence mode="wait">
              {selectedRole === 'student' && (
                <motion.div key="s" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
                  exit={{ opacity: 0, height: 0 }} className="flex flex-col gap-4 overflow-hidden">
                  <Field label="Course" error={errors.course?.message}>
                    <input type="text" placeholder="e.g. MCA, B.Tech CSE"
                      {...register('course')} className={iCls(errors.course)} />
                  </Field>
                  <Field label="Student ID (optional)" error={errors.studentId?.message}>
                    <input type="text" placeholder="e.g. KTU2024CS001"
                      {...register('studentId')} className={iCls(errors.studentId)} />
                  </Field>
                </motion.div>
              )}
              {selectedRole === 'employer' && (
                <motion.div key="e" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
                  exit={{ opacity: 0, height: 0 }} className="flex flex-col gap-4 overflow-hidden">
            ...
            <button type="submit" disabled={loading} data-hover
              ...
              style={{ background: `linear-gradient(90deg, ${role.color} 0%, #a855f7 50%, #38bdf8 100%)` }}
            >
              {loading ? (
